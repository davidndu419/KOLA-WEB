...
export function CorrectionSheet({
  ...
  const [lastCorrection, setLastCorrection] = useState<{ amount: number; type: string; local_id: string } | null>(null);
  ...
  return (
    <>
      ...
      <QuickReceiptSheet
        isOpen={!!lastCorrection}
        onClose={() => setLastCorrection(null)}
        type="correction"
        amount={lastCorrection?.amount || 0}
        title="Transaction Corrected"
        subtitle={`Correction to transaction: #${lastCorrection?.local_id.slice(0, 8).toUpperCase()}`}
